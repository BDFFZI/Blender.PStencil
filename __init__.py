...
class UpdatePStencil(bpy.types.Operator):
    bl_idname = "paint.updatepstencil"
    bl_label = "UpdatePStencil"
    bl_description = """使用说明：
...
"""
    ...
    def execute(self, context):
        ...
        # 笔刷初始化
        bpy.ops.wm.tool_set_by_id(name="builtin_brush.Draw")  # 仅支持自由线绘制模式
        ...
        self.report({"INFO"}, "镂版已更新：" + f"{end - start:.3f}s")
        return {"FINISHED"}
...
class PStencilPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(context.area.spaces.active, "lock_camera", text="锁定相机视图")
        ...
        layout.operator(UpdatePStencil.bl_idname, text="更新镂版并进入相机视图")
